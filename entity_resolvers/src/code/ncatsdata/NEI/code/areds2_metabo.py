#!/usr/bin/env python3
"""
areds_sri_resolver.py — Resolve AREDS metabolite names using Translator SRI

Usage:
    python areds_sri_resolver.py \
        --input AREDS_sig_metabolites.csv \
        --output AREDS_sig_metabolites_sri.tsv
...
def setup_logger(level: str = "INFO") -> logging.Logger:
    logger = logging.getLogger("areds_sri_resolver")
...
# --------------------------------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description="Resolve AREDS2 metabolites via SRI Name Resolver.")
    ap.add_argument(
        "--input",
        default="AREDS2_sig_metabolites.csv",
        help="Input CSV with CHEMICAL_NAME column (default: AREDS2_sig_metabolites.csv)",
    )
    ap.add_argument(
        "--output",
        default="AREDS2_sig_metabolites_sri.csv",
        help="Output CSV with SRI mappings (default: AREDS2_sig_metabolites_sri.csv)",
    )
    ap.add_argument(
        "--log_level",
        default="INFO",
    ...
    logger.info(f"Reading input CSV: {in_path}")
