...
```mermaid
flowchart TD
    START[📄 Raw Patient Data<br/>cureid_cases_VERSION.json<br/><i>~200 cases with free-text symptoms</i>] --> SRI
    
    SRI[🤖 Step 1: Automated Mapping<br/>SRI Name Resolver API<br/><i>Queries biomedical databases</i>]
    
    SRI --> EXACT[✅ Exact Matches<br/><i>~60% of terms</i><br/>Auto-accepted]
    SRI --> NONEXACT[⚠️ Non-Exact Matches<br/><i>~40% of terms</i><br/>Need review]
...
|-------|-------|--------|------|
| **SRI Resolution** | 200 JSON records | ~150 unique nodes | ~5-10 min |
| **Exact Matches** | 150 nodes | ~90 auto-accepted | instant |
| **AI Review** | 60 non-exact | 60 curated | ~5 min |
| **Human QC** | 60 AI-reviewed | 60 validated | ~30-60 min |
| **Final Apply** | 200 records | 250+ edges | ~2 min |

**Total Time:** ~45-75 minutes per dataset (most time is human QC)
...
### KGX Export
**Format:** Knowledge Graph Exchange (Translator standard)  
**Use:** Integration with Translator knowledge graphs

### Neo4j Import
**Format:** TSV nodes + edges  
**Use:** Graph database queries and visualization

### Data Analysis
**Format:** Structured TSV/Excel  
**Use:** Statistical analysis, reporting, research

---

## 🔑 Key Benefits

1. **Scalability**: Processes 200+ cases in <1 hour
2. **Quality**: Multi-layer validation catches errors
3. **Consistency**: Standardized ontology terms
4. **Transparency**: Full audit trail of decisions
5. **Reproducibility**: Same input → same output
6. **Interoperability**: Compatible with Translator ecosystem
