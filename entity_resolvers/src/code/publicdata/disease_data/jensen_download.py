#!/usr/bin/env python
# jensen_download.py - Download Jensen disease associations with metadata/hash tracking only
...
def head_metadata(url: str):
    ...
    try:
        ...
        logging.warning(f"Jensen HEAD failed for {url}: {e}")
    return out
...
class JensenDiseaseDownloader:
    def __init__(self, full_config):
        ...
        self.tsv_file.parent.mkdir(parents=True, exist_ok=True)
    ...
    def _should_skip(self, old_meta, remote_meta):
    ...
    def run(self):
        ...
        if self._should_skip(old_meta, remote_meta):
            logging.info("✅ Skipping Jensen download — remote metadata unchanged.")
            meta = {
                **old_meta,
        ...
        response = requests.get(self.download_url, stream=True, timeout=(30, 180))
        ...
        new_hash = sha256sum(temp_file)
        old_hash = old_meta.get("sha256")
        changed = True

        if self.tsv_file.exists() and old_hash == new_hash:
            changed = False
        ...
        meta = {
            ...
            "raw_file": str(self.tsv_file),
            "last_modified": remote_meta.get("last_modified"),
            "last_modified_iso": remote_meta.get("last_modified_iso"),
            "etag": remote_meta.get("etag"),
            "content_length": remote_meta.get("content_length"),
            "sha256": old_hash if not changed else sha256sum(self.tsv_file),
            "changed": changed,
        }
        json.dump(meta, open(self.metadata_file, "w"), indent=2)
