...
def _source_label(item) -> str:
    ...
    if item.source_file:
        return os.path.basename(item.source_file)
    sheet = item.sheet
    ...
    if sheet in _EXPERIMENT_RECOGNIZED_SHEETS:
        return os.path.basename(experiment_files[0]) if len(experiment_files) == 1 else "Experiment workbook(s)"
    if sheet in _STATS_RECOGNIZED_SHEETS:
        return os.path.basename(stats_files[0]) if len(stats_files) == 1 else "Stats results workbook(s)"
