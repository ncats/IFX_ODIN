...
def register_hmd_human_phenotype(
    *,
    dest: Path,
    ...
    version = MGI_HMD_FILE_NAME
    work_dir = dest / source / dataset / "pending"
    local_path, metadata = download_url(MGI_HMD_URL, work_dir, timeout=timeout)
    version_date = metadata.get("version_date")
    if not version_date:
        raise ValueError(f"Could not determine MGI HMD version_date from Last-Modified for {MGI_HMD_URL}")
    ...
    manifest = build_source_snapshot_manifest(
        ...
        upstream_urls=[MGI_HMD_URL],
        files=[file_entry(final_path, metadata.get("final_url") or MGI_HMD_URL, storage_uri, metadata.get("content_type"))],
        extra={
            "version_method": {
                ...
                "evidence": {
                    "url": metadata.get("final_url") or MGI_HMD_URL,
